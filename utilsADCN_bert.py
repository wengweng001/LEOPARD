...
from ACDCDataManipulator_bert import DataManipulator
...
class sourceAndTargetAmazonLoader():
    def __init__(self,datasetSource,datasetTarget):
         self.getDataset(datasetSource,datasetTarget)
    def getDataset(self,datasetSource,datasetTarget):
        if datasetSource=="AM1":
            S = DataManipulator()  # 5150
            S.load_amazon_review_all_beauty()
            print ('Data Souce : Beauty')
        elif datasetSource=="AM2":
            S = DataManipulator()
            S.load_amazon_review_books()
            print('Data Souce : Book')
        elif datasetSource=="AM3":
            S = DataManipulator()
            S.load_amazon_review_industrial_scientific()
            print('Data Souce : Industrial')
        elif datasetSource=="AM4":
            S = DataManipulator()
            S.load_amazon_review_luxury_beauty()
            print('Data Souce : Luxury')
        else:
            S = DataManipulator()
            S.load_amazon_review_magazine_subscription()
            print('Data Souce : Magazine')

        if datasetTarget=="AM1":
            T = DataManipulator()  # 5150
            T.load_amazon_review_all_beauty()
            print('Data Target : Beauty')
        elif datasetTarget=="AM2":
            T= DataManipulator()
            T.load_amazon_review_books()
            print('Data Target : Book')
        elif datasetTarget=="AM3":
            T= DataManipulator()
            T.load_amazon_review_industrial_scientific()
            print('Data Target : Industrial')
        elif datasetTarget=="AM4":
            T= DataManipulator()
            T.load_amazon_review_luxury_beauty()
            print('Data Target : Luxury')
        else:
            T= DataManipulator()
            T.load_amazon_review_magazine_subscription()
            print('Data Target : Magazine')
        self.S=S
        self.T=T


class amazonDataLoader(object):
    def __init__(self, data, nEachClassSamples=None):

        self.loadDataAmazon(data, nEachClassSamples)

    def loadDataAmazon(self, data, nEachClassSamples):
        datadaray = data.data.datasets.values #takes the data array from data extracted from amazonData
        datatensor = torch.from_numpy(datadaray) #convert datadaray to tensor
        data = datatensor.float()
        self.unlabeledData = data[:, 0:-1]
        label = data[:, -1] - 1  # this is to adjust new label into the range 0 to nClass-1
        self.unlabeledLabel = label.long()

        # shuffle index of the Data
        indices = torch.randperm(data.shape[0])
        self.unlabeledData = self.unlabeledData[indices]
        self.unlabeledData = self.unlabeledData[indices]

        # this part is to check whether the frequency of each label (if not none), higher than
        # example tensor([ 115,   58,  109,  321, 4547]), label 1 only happen 58 times, we cannot take a sample in
        # class 1 more than 58
        freq = torch.bincount(self.unlabeledLabel)
        arrayEachClassSamples = torch.round(freq / 10)
        # ass = int(arrayEachClassSamples[2])
        if nEachClassSamples is not None:
            if nEachClassSamples > min(
                    torch.bincount(self.unlabeledLabel)):  # example tensor([ 115,   58,  109,  321, 4547])
                nEachClassSamples = int(min(torch.bincount(self.unlabeledLabel)))
        self.nData = data.shape[0]

        self.nInput = self.unlabeledData.shape[1]
        self.nOutput = torch.unique(self.unlabeledLabel).shape[0]

        self.classes = torch.unique(self.unlabeledLabel).tolist()
        self.unLabeledData = self.unlabeledData.shape[0]

        if nEachClassSamples is not None:
            selectedLabeledData = torch.Tensor(int(sum(arrayEachClassSamples)), self.nInput)
            selectedLabeledLabel = torch.LongTensor( int(sum(arrayEachClassSamples))  )
            ...
            for iClass in self.classes:
                ...
                for iData in range(0, self.unLabeledData):
                    # print(iData)
                    if self.unlabeledLabel[iData] == iClass:
                        selectedLabeledData[idx] = self.unlabeledData[iData]
                        selectedLabeledLabel[idx] = self.unlabeledLabel[iData]
                    ...
                    if dataCount == int(arrayEachClassSamples[iClass]):
                        break

            remainderData = deleteRowTensor(self.unlabeledData, selectedDataIdx, 2)
            remainderLabel = deleteRowTensor(self.unlabeledLabel, selectedDataIdx, 2)
            ...
            print('around',round((self.nLabeledData/self.nData*100),2), 'percent of data')
            print('Number of unlabeled data: ', self.nUnlabeledData)
            # print('Number of unlabeled data batch: ', self.nBatch)
        else:
            print('============Target Data==========')
            self.unlabeledData = self.unlabeledData
            self.unlabeledLabel = self.unlabeledLabel
            self.nUnlabeledData = self.unlabeledData.shape[0]

            print('Number of output: ', self.nOutput)
            print('Number of labeled data: ', 0)
            print('Number of unlabeled data: ', self.nUnlabeledData)
            # print('Number of unlabeled data batch: ', self.nBatch)
            print('Around: ', (   round(self.nUnlabeledData/self.nData*100,2)   ))
    ...
    def maxMinNormalization(self):
        self.labeledData = torch.from_numpy(
            preprocessing.minmax_scale(self.labeledData, feature_range=(0.001, 1))).float()
...
class amazonDataLoaderOld(object):
    def __init__(self, data, nEachClassSamples=None):
    ...
    def loadDataAmazon(self, data, nEachClassSamples):
        datadaray = data.data.datasets.values
        datatensor = torch.from_numpy(datadaray)
        data = datatensor.float()
        ...
        # example tensor([ 115,   58,  109,  321, 4547]), label 1 only happen 58 times, we cannot take a sample in
        # class 1 more than 58
        freq = torch.bincount(self.labeledLabel)
        # arrayEachClassSamples = torch.round(freq / 10)
        # ass = int(arrayEachClassSamples[2])
        if nEachClassSamples is not None:
            if nEachClassSamples > min(
                    torch.bincount(self.labeledLabel)):  # example tensor([ 115,   58,  109,  321, 4547])
                nEachClassSamples = int(min(torch.bincount(self.labeledLabel)))
        self.nData = data.shape[0]
        # self.nBatch = int(self.nData / self.batchSize)
        ...
        if nEachClassSamples is not None:
            ...
            self.unlabeledData = self.labeledData
            self.unlabeledLabel = self.labeledLabel
            self.nUnlabeledData = self.nLabeledData
            print('========Target Data: ===========')
            print('Number of output: ', self.nOutput)
            print('Number of labeled data: ', 0)
            print('Number of unlabeled data: ', self.nUnlabeledData)
...
class amazonDataLoaderUniform(object):
    def __init__(self, data, nEachClassSamples=None):
    ...
    def loadDataAmazon(self, data,nEachClassSamples):
        ...
        if nEachClassSamples is not None:
            nEachClass = []
            for i in freq:
                if i < nEachClassSamples:
                    nEachClass.append(i)
                else:
                    nEachClass.append(nEachClassSamples)
            sourcedata = np.sum(nEachClass)
            selectedLabeledData = torch.Tensor(sourcedata, self.nInput)
            selectedLabeledLabel = torch.LongTensor(sourcedata)

            idx = 0
            selectedDataIdx = []
            for iClass,nC in zip(self.classes,nEachClass):
                # print(iClass)
                # print(nC)
                dataCount = 0
                ...
                for iData in range(0, self.nLabeledData):
                    ...
                    if dataCount == nC:
                        break
            print(torch.bincount(selectedLabeledLabel))
            remainderData = deleteRowTensor(self.labeledData, selectedDataIdx, 2)
            remainderLabel = deleteRowTensor(self.labeledLabel, selectedDataIdx, 2)
...
class amazonDataLoaderRev(object):
    def __init__(self, data, nPortion=None):

        self.loadDataAmazon(data, nPortion)


    def loadDataAmazon(self, data, nPortion):
        datadaray = data.data.datasets.values
        datatensor = torch.from_numpy(datadaray)
        data = datatensor.float()
        self.labeledData = data[:, 0:-1]
        ...
        if nPortion is not None:
            ...
            for i in freq:
                n += int(i*nPortion)
            selectedLabeledData = torch.Tensor(n, self.nInput)
            selectedLabeledLabel = torch.LongTensor(n)
            ...
            for iClass in self.classes:
                ...
                for iData in range(0, self.nLabeledData):
                    ...
                    if dataCount == int(freq[iClass]*nPortion):
                        break
...
class MnistLoaders(object):

    def __init__(self, labeldSamples, unlabeldSamples, testingBatchSize=1000, nEachClassSamples=None):
        # np.random.seed(0)
        # torch.manual_seed(0)
        # random.seed(0)
        # labeldSamples 10000,28,28 ; test_source
        # unlabeldSamples 60000,28,28 ; train_source
    ...
    def loadData(self, labeldSamples, unlabeldSamples, nEachClassSamples):
        # labeled data
        labeledData = torch.Tensor(labeldSamples.train_labels.shape[0], 1, 28, 28)
        labeledLabel = torch.LongTensor(labeldSamples.train_labels.shape[0])

        for idx, example in enumerate(labeldSamples):
            labeledData[idx] = example[0]
            labeledLabel[idx] = example[1]
        sizelabeledData = np.shape(labeledData)
        sizelabeledLabel = np.shape(labeledLabel)
        ...
        # unlabeled data
        unlabeledData = torch.Tensor(unlabeldSamples.train_labels.shape[0], 1, 28, 28)
        unlabeledLabel = torch.LongTensor(unlabeldSamples.train_labels.shape[0])
    ...
    def createTask(self, nTask=2, taskList=[], taskType=1):
        ...
        for iTask in range(0, nTask):
            ...
            # load labeled data
            taskLabeledData = transformedLabeledData[
                              (iTask) * self.nLabeledDataPerTask:(iTask + 1) * self.nLabeledDataPerTask]
            taskLabeledLabel = transformedLabeledLabel[
                               (iTask) * self.nLabeledDataPerTask:(iTask + 1) * self.nLabeledDataPerTask]

            # load unlabeled data
            taskUnlabeledData = transformedUnlabeledData[
                                (iTask) * self.nUnlabeledDataPerTask:(iTask + 1) * self.nUnlabeledDataPerTask]
            taskUnlabeledLabel = transformedUnlabeledLabel[
                                 (iTask) * self.nUnlabeledDataPerTask:(iTask + 1) * self.nUnlabeledDataPerTask]

            if self.nUnlabeledDataPerTask != taskUnlabeledData.shape[0] or self.nLabeledDataPerTask != \
                    taskLabeledData.shape[0]:
            ...
            if taskType == 1:
                ...
                for idx, _ in enumerate(taskLabeledData):
                    taskLabeledData[idx] = Rotate(random.randint(taskList[iTask][0], taskList[iTask][1]))(
                        taskLabeledData[idx])

                # rotate unlabeled data
                for idx, _ in enumerate(taskUnlabeledData):
                    taskUnlabeledData[idx] = Rotate(random.randint(taskList[iTask][0], taskList[iTask][1]))(
                        taskUnlabeledData[idx])
                ...
                for iClass in taskList[iTask]:
                    # split labeled data
                    taskLabeledDataSplit = torch.cat(
                        (taskLabeledDataSplit, transformedLabeledData[transformedLabeledLabel == iClass]), 0)
                    taskLabeledLabelSplit = torch.cat(
                        (taskLabeledLabelSplit, transformedLabeledLabel[transformedLabeledLabel == iClass]), 0)

                    # split unlabeled data
                    taskUnlabeledDataSplit = torch.cat(
                        (taskUnlabeledDataSplit, transformedUnlabeledData[transformedUnlabeledLabel == iClass]), 0)
                    taskUnlabeledLabelSplit = torch.cat(
                        (taskUnlabeledLabelSplit, transformedUnlabeledLabel[transformedUnlabeledLabel == iClass]), 0)
...
# This class combine two traininig and testing datasets and shuffle both of them
# This configuration is used only for data Mnist and USPS
class generalExtractorMnistUsps(object):
    def __init__(self, partialDataOne, partialDataTwo):
        self.loadData(partialDataOne, partialDataTwo)

    def loadData(self, partialDataOne, partialDataTwo):
        # labeled data
        partialDataOneTensor = torch.Tensor(partialDataOne.dataset_size, 1, 28, 28)
        partialDataOneLabelTensor = torch.LongTensor(partialDataOne.dataset_size)
        partialDataTwoTensor = torch.Tensor(partialDataTwo.dataset_size, 1, 28, 28)
        partialDataTwoLabelTensor = torch.LongTensor(partialDataTwo.dataset_size)

        for idx, example in enumerate(partialDataOne):
            partialDataOneTensor[idx] = example[0]
            partialDataOneLabelTensor[idx] = example[1]

        for idx, example in enumerate(partialDataTwo):
            partialDataTwoTensor[idx] = example[0]
            partialDataTwoLabelTensor[idx] = example[1]

        joinedData = torch.cat((partialDataOneTensor, partialDataTwoTensor), dim=0)
        joinedLabel = torch.cat((partialDataOneLabelTensor, partialDataTwoLabelTensor), dim=0)

        # get size of the Data
        sizeData = partialDataOne.dataset_size + partialDataTwo.dataset_size

        # shuffle index of the Data
        indices = torch.randperm(sizeData)
        self.unlabeledData = joinedData[indices]
        self.unlabeledLabel = joinedLabel[indices]


# This class combine two traininig and testing datasets and shuffle both of them
# This configuration is used only for data Mnist and USPS
class generalExtractorAmazonDataset(object):
    def __init__(self, partialDataOne, partialDataTwo):
        # partialDataTwo=dataStreamSource
        self.loadData(partialDataOne, partialDataTwo)

    def loadData(self, partialDataOne, partialDataTwo):
        DataTr = partialDataOne[:-1]
        dim = partialDataOne.shape
        DataLabel = partialDataOne[(dim[0] - 1):dim[0]]

        # labeled data
        # partialDataOneTensor  = torch.Tensor(partialDataOne.data.datasets.shape[0],partialDataOne.data.datasets.shape[1]-1)
        partialDataOneTensor = torch.Tensor(DataTr.shape[0], DataTr.shape[1])
        partialDataOneLabelTensor = torch.LongTensor(DataLabel.shape[0], DataLabel.shape[1])
        # partialDataTwoTensor  = torch.Tensor(partialDataTwo.dataset_size,1,28,28)
        # partialDataTwoLabelTensor = torch.LongTensor(partialDataTwo.dataset_size)

        for idx, example in enumerate(partialDataOneTensor):
            partialDataOneTensor[idx] = example[0]
            # partialDataOneLabelTensor[idx] = example[1]

        for idx, example in enumerate(partialDataOneLabelTensor):
            partialDataOneLabelTensor[idx] = example[0]
            # partialDataTwoLabelTensor[idx] = example[1]
...
class meanStdCalculator(object):
    ...
    def updateMeanStd(self, data, cnt=1):
        ...
        self.count += cnt
        self.mean = self.mean_old + (data - self.mean_old) / (
        (self.count + 0.0001))  # np.divide((data-self.mean_old),self.count + 0.0001)
...
# This Class takes portion of USPS dataset based on portion of labeled data
# A portion of labeled data is selected randomly from the whole USPS dataset
# nEachClassSamples means that we take a number of samples from each class
# Thus for example, if there is 10 classes and nEachClassSamples is 50, we will take 500 data
# All number of data (e.g. 500) is a portion from the whole dataset
class generalLoader(object):
    def __init__(self, partialDataOne, partialDataTwo, nEachClassSamples=None):
        self.loadData(partialDataOne, partialDataTwo, nEachClassSamples)

    # loadData is a method to divide Data into two kinds of data which are initialTrainingData  dan unlabeledData
    # return value: intialTrainingData and unlabeledData

    # nEachClassSamples means that we take a number of samples from each class
    # Data is formed from join #partialDataOne will be joined together with partialDataTwo to form Data. Both of partialDataOne and partialDataTwo
    # are taken from the Internet
    # Once Data (and its Label) are formed, we create initialTrainingData dan unlabeledData
    # initialTrainingData is the number of samples from each Class selected from Data,
    # whereas unlabeledData are the rest(unselected samples of Data)
    # initialTrainingData is used for warming up phase
    # unlabeledData is used for sequential process

    def loadData(self, partialDataOne, partialDataTwo, nEachClassSamples):
        # labeled data
        partialDataOneTensor = torch.Tensor(partialDataOne.dataset_size, 1, 28, 28)
        partialDataOneLabelTensor = torch.LongTensor(partialDataOne.dataset_size)
        partialDataTwoTensor = torch.Tensor(partialDataTwo.dataset_size, 1, 28, 28)
        ...
        # shuffle index of the Data
        indices = torch.randperm(sizeData)
        Data = joinedData[indices]
        Label = joinedLabel[indices]

        # Now we aim to take a portion from Data and Label by taking the same number of data from each class

        self.classes = torch.unique(Label).tolist()
        self.nOutput = torch.unique(Label).shape[0]
        nLabel = Label.shape[0]

        # taking portion of Data as initialTrainingData and the remainingTrainingData for sequential process
        if nEachClassSamples is not None:
            self.labeledData = torch.Tensor(self.nOutput * nEachClassSamples, 1, 28, 28)
            self.labeledLabel = torch.LongTensor(self.nOutput * nEachClassSamples)
            ...
            for iClass in self.classes:
                ...
                for iData in range(0, nLabel):
                    # data selection for initialTrainingData rename int labeledData
                    if joinedLabel[iData] == iClass:
                        self.labeledData[counter] = Data[iData]
                        self.labeledLabel[counter] = Label[iData]
                        ...
                        selectedDataIdx.append(iData)
                        iData += 1
                        if dataCountAllLabel == self.nOutput * nEachClassSamples:
                            self.unlabeledData = deleteRowTensor(Data, selectedDataIdx, 2)
                            self.unlabeledLabel = deleteRowTensor(Label, selectedDataIdx, 2)

                    if dataCount == nEachClassSamples:
                        break
            print('===============SOURCE DATA===============')
            print('Number of output for Source Data: ', self.nOutput)
            print('Number of labeled data for warming up phase is ', self.labeledData.shape[0],
                  'samples, which is around ', round((100 * dataCountAllLabel / sizeData), 2), 'percent of total Data')
            print('Number of unlabeled data: ', self.unlabeledData.shape[0])
        else:
            self.unlabeledData = joinedData
            self.unlabeledLabel = joinedLabel
            print('===============TARGET DATA===============')
            print('Number of output for Target Data: ', self.nOutput)
            print('There is no labeled data')
            print('Number of unlabeled data: ', self.unlabeledData.shape[0])


class generalLoader1(object):
    def __init__(self, partialDataOne, partialDataTwo, testingBatchSize=1000, nEachClassSamples=None):
        self.batchSize = testingBatchSize
        self.loadData(partialDataOne, partialDataTwo, nEachClassSamples)
    ...
    def loadData(self, partialDataOne, partialDataTwo, nEachClassSamples):
        # labeled data
        partialDataOneTensor = torch.Tensor(partialDataOne.test_data.shape[0], 1, 28, 28)
        partialDataOneLabelTensor = torch.LongTensor(partialDataOne.test_data.shape[0])
        partialDataTwoTensor = torch.Tensor(partialDataTwo.train_data.shape[0], 1, 28, 28)
        partialDataTwoLabelTensor = torch.LongTensor(partialDataTwo.train_data.shape[0])
        ...
        # get size of the Data
        # sizeData=partialDataOne.dataset_size+partialDataTwo.dataset_size
        sizeData = partialDataOne.test_data.shape[0] + partialDataTwo.train_data.shape[0]
        ...
        # taking portion of Data as initialTrainingData and the remainingTrainingData for sequential process
        if nEachClassSamples is not None:
            self.initialTrainingData = torch.Tensor(self.nOutput * nEachClassSamples, 1, 28, 28)
            self.initialLabeledData = torch.LongTensor(self.nOutput * nEachClassSamples)
...
class uspsLoader(object):
    ...
    def loadData(self, labeldSamples, unlabeldSamples, nEachClassSamples):
        ...
        labeledData = torch.Tensor(1860, 1, 28, 28)
        labeledLabel = torch.LongTensor(1860)
        ...
        # unlabeled data
        unlabeledData = torch.Tensor(7438, 1, 28, 28)
        unlabeledLabel = torch.LongTensor(7438)
...
class mnistLoader(object):
    ...
    def loadData(self, labeldSamples, unlabeldSamples, nEachClassSamples):
        # labeled data
        labeledData = torch.Tensor(labeldSamples.label, 1, 28, 28)
...
class omniglotLoader(object):
    ...
    def loadData(self, dataTrain, labelTrain, propLabeled, propUnlabeled, propTest, nDataPerClass, nTask,
                 nClassperTask):
        finalLabeledData = {}
        finalLabeledLabel = {}
...
class cifarLoader(object):
    ...
    def createTask(self, nTask=2, taskList=[], taskType=1):
        ...
        for iTask in range(0, nTask):
            ...
            finalUnlabeledData[iTask] = taskUnlabeledData[self.batchSize:]
            finalUnlabeledLabel[iTask] = taskUnlabeledLabel[self.batchSize:]
            self.taskIndicator = self.taskIndicator + (
                        iTask * torch.ones(int(finalUnlabeledData[iTask].shape[0] / self.batchSize)).long()).tolist()
...
def getlistchunk(nData_S, nData_T, nBatch):
    seq = random.sample(range(0, nData_S), nData_S)
    avg = len(seq) / float(nBatch)
    out1 = []
    last = 0.0

    while last < len(seq):
        out1.append(seq[int(last):int(last + avg)])
        last += avg

    seq = random.sample(range(0, nData_T), nData_T)
    avg = len(seq) / float(nBatch)
    out2 = []
    last = 0.0

    while last < len(seq):
        out2.append(seq[int(last):int(last + avg)])
        last += avg
...
# def getlistchunk (nData_S,nData_T,nBatch):
#     if (round(nData_S/nBatch)*nBatch < nData_S):
#         sizeS=round(nData_S/nBatch)+1
#     else:
#         sizeS=round(nData_S/nBatch)
#
#     if (round(nData_T/nBatch)*nBatch < nData_T):
#         sizeT=round(nData_T/nBatch)+1
#     else:
#         sizeT=round(nData_T/nBatch)
#
#     listchunk_s = list(chunks(random.sample(range(0, nData_S), nData_S), sizeS))
#     listchunk_t = list(chunks(random.sample(range(0, nData_T), nData_T), sizeT))
#     return listchunk_s,listchunk_t
...
def evenly(numComponents, numBatch):
    div = numComponents / numBatch
    deviden = int(np.floor(div))
    list = [deviden] * numBatch
    theRest = numComponents % numBatch
    if theRest != 0:
...
def listCollectS(listSource):
    startIndexS = 0
    listCollect = []
    listall = list(range(0, sum(listSource)))
    for iBatch in range(0, len(listSource)):
        endIndexS = startIndexS + listSource[iBatch]
        lists = listall[startIndexS:endIndexS]
        listCollect.append(lists)
