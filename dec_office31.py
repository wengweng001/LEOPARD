...
class DEC_AE(nn.Module):
    ...
    """
    def __init__(self, num_classes, base_net='resnet34', device='cpu'):
        ...
        self.alpha = 1.0
        self.clusterCenter = nn.Parameter(torch.zeros(num_classes,500)).to(device)
...
class DEC:
    ...
    def pretrain(self, data, labels, epochs, batch, device, typerun, network_name):
        self.batchsize = batch
        self.device = device
        
        dec_ae = DEC_AE(self.n_clusters, network_name, device=self.device)
