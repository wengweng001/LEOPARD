from LeopardBasic_dataset import ADCN
import numpy as np
import torch
...
def run_leopard_office(params):
    #Setting========
    TypeAblation=params.source+'2'+params.target


    # create folder to save performance file
    file_path = 'images/best_model'
    if not os.path.exists(file_path):
        os.makedirs(file_path)
    file_path = 'saved_model/best_model'
    if not os.path.exists(file_path):
        os.makedirs(file_path)
    if not os.path.exists('saved_model/pre_trained_model'):
        os.makedirs('saved_model/pre_trained_model')
    file_path = 'outputs/office/leopard+'
    if not os.path.exists(file_path):
        os.makedirs(file_path)
    ...
    model_stamp_cnn = (TypeAblation + '_' + str(portion) +'Portion_' 
        # + str(noOfEpochInitializationCluster) + 'initEpochs'
        + '_batchsize' + str(batch)
        + '_lr(SGD)' + str(lr))

    paramstamp = ('Leopard++_office_' + TypeAblation +str(nRoundTest)+'Times_'+str(portion)+'Portion_' + str(noOfEpochInitializationCluster) + 'initclusepoch_'
    ...
    if os.path.isfile(result_dir):
        ...
        for iRoundTest in range(nRoundTest):
            ...
            nClass=31
            testingBatchSize=64
            # nBatch1 = int(len(dataStreamSource.dataset2)/testingBatchSize)
            # nBatch2 = int(dataStreamTarget.total_count/testingBatchSize)
            # print('Batch size wrt two datastreams:', nBatch1, nBatch2)
            nBatch = 10
            ...
            ADCNnet = ADCN(dataStreamSource.nOutput, nInitCluster, desiredLabels=list(range(0,31)), alpha_kl=alpha1, alpha_dc=alpha2, LR=lr, basenet=params.backbone)  # Initialization of ADCN structure
            ADCNnet.model_cnn   = model_stamp_cnn
            ...
            for iBatch, (a,b) in enumerate(zip(dataset1, dataset2)):
                ...
                Y_true = Y_true + ADCNnet.trueClassLabel
