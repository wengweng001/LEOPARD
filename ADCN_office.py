...
def run_adcn_office(params):
    ...
    paramstamp = ('ADCN-office-' + TypeAblation + str(nRoundTest) + 'Times_' + str(portion) + 'Portion_'
                      + str(noOfEpochInitializationCluster) + 'initEpoch_'
                      + str(noOfEpochPrequential) + 'ClusEpochs_' + str(nEpochKLL1) + 'L1Epochs_'
    ...
    if os.path.isfile(result_dir):
        ...
        for iRoundTest in range(nRoundTest):
            ...
            ADCNnet = ADCN(dataStreamSource.nOutput, nInitCluster, nInput = 1000, nHiddenNode = 500, desiredLabels=list(range(0,31)), LR=lr)  # Initialization of ADCN structure
            ADCNnet.ADCNcnn = ConvAe(base_net=params.backbone)
