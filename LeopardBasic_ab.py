...
class ADCNMPL(nn.Module):
    ...
    def trainDC(self, x, y, x1, x2):
        ...
        if self.mmd_d:
            ...
            # sigmaOPT.requires_grad = True
            # sigma0OPT = MatConvert(np.sqrt(np.random.rand(1) * 0.02), device, dtype)
            # sigma0OPT.requires_grad = True
            ...
            for i in range(batchs):
                ...
                X = torch.cat([minibatch_x1, minibatch_x2], 0)

                # clear the gradients of all optimized variables
                optimizer.zero_grad()

                # forward encoder CNN
                feat_x = self.ADCNcnn(X)
                ...
                # mmd loss
                TEMP = MMDu(feat_x, minibatch_x1.shape[0], X.view(X.shape[0],-1), sigma, sigma0_u, ep)
            ...
            # Fetch training data
            N_per = 100
            N1 = min(x1.shape[0], x2.shape[0])
            S = torch.cat([x1[:N1].cpu(),x2[:N1].cpu()],0).to(self.device)
            Sv = S.view(2*N1,-1)
            # Run two-sample test (MMD-D) on the training set
            with torch.no_grad():
                # forward encoder CNN
                feat_s = self.ADCNcnn(S.to(self.device))
                ...
                for iLayer,_ in enumerate(self.ADCNae):
                    ...
                    obj     = obj.to(self.device)
                    feat_s  = obj(feat_s)
            h_u, threshold_u, mmd_value_u = TST_MMD_u(feat_s, N_per, N1, Sv, sigma, sigma0_u, ep, alpha, device, dtype)
            ...
            if self.mmd_o:
                ...
                optimizer_sigma0 = torch.optim.SGD([sigma0], lr=self.lr)
